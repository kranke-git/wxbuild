...
def find_base_rmy_epw(wxbuild_root: str | Path, city: str) -> Path:
    """Pick the base EPW from <city>/Chosen_scenario (use the single EPW found).

    This is treated as the FRMY (plotted base file).
    """
    chosen_dir = Path(wxbuild_root) / "epwdata" / city / "Chosen_scenario"
    if not chosen_dir.exists():
        raise FileNotFoundError(f"Missing Chosen_scenario folder: {chosen_dir}")
    epws = sorted(chosen_dir.glob("*.epw"))
    if not epws:
        raise FileNotFoundError(f"No EPWs in: {chosen_dir}")
    return epws[0]
...
def plot_uwg_runs_vs_base(
    ...
    Plot each run against the FRMY EPW in:
      <wxbuild_root>/epwdata/<city>/Chosen_scenario/

    FRMY is always dark grey. UWG runs use a red palette.
    ...
    plt.plot(base["hoy"], base["dbt"], label="FRMY", linewidth=2.0, color=COL_BASE)

    for (p, df), c in zip(run_dfs, uwg_cols):
        plt.plot(df["hoy"], df["dbt"], label=p.stem, linewidth=2.2, color=c)

    plt.title(f"{city} — Dry-bulb temperature: FRMY vs UWG runs")
    ...
    for (p, df), c in zip(run_dfs, uwg_cols):
        ...
        plt.title(f"{city} — Δ Dry-bulb (UWG − FRMY) | {p.name}")
        plt.xlabel("Hour of year")
    ...
    plt.plot(monthly_base.index, monthly_base.values, marker="o", linewidth=2.4, color=COL_BASE, label="FRMY")
    for (p, df), c in zip(run_dfs, uwg_cols):
    ...
    plt.title(f"{city} — Monthly mean Δ dry-bulb (UWG − FRMY)")
    plt.xlabel("Month")
