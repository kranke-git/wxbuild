...
def _detect_city_from_epwdata(wxbuild_root: str | Path) -> str:
    """If only one city exists under epwdata, return it. Otherwise raise."""
    wxbuild_root = Path(wxbuild_root)
    epwdata = wxbuild_root / "epwdata"
    cities = sorted([p.name for p in epwdata.iterdir() if p.is_dir()]) if epwdata.exists() else []
    if len(cities) == 1:
        return cities[0]
    raise RuntimeError(
        f"Multiple cities found under {epwdata}. Please pass city explicitly. Found: {cities}"
    )
...
if __name__ == "__main__":
    # Default behavior in Colab: show plots and save them under the city uwg folder.
