...
def _plot_corr_heatmap(df: pd.DataFrame, features: list[str], targets: list[str], title: str) -> pd.DataFrame:
    ...
    plt.figure(figsize=(max(10, 0.8 * len(targets) + 4), max(6, 0.6 * len(features) + 2)))
...
def _perm_importance_rankings(
    ...
    if not per_target:
        raise RuntimeError("No targets produced permutation importance results.")
...
def analyze_and_rank(
    ...
    if make_plots:
        ...
        print("\n✅ Overall strongest knobs across targets (normalized mean importance):")
        print(overall_rank[["mean_importance"]].head(12).to_string())
