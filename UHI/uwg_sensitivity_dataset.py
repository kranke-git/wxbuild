...
def _plot_corr_heatmap(df: pd.DataFrame, features: list[str], targets: list[str], title: str) -> pd.DataFrame:
    ...
    # If correlations are all NaN (e.g., parameters never changed), skip plotting
    if not np.isfinite(corr.values).any():
        return corr
...
def analyze_and_rank(
    ...
    # Silence common numeric warnings in small-sample exploratory runs
    ...
    # Summary sentences for student-facing printout
    # 1) Which parameters did not change across runs
    unchanged = []
    for f in features:
        s = pd.to_numeric(df[f], errors="coerce")
        if s.nunique(dropna=True) <= 1:
            unchanged.append(FEATURE_LABELS.get(f, f))
    if unchanged:
        print("\nParameters not changed across runs (no variation): " + ", ".join(unchanged) + ".")
    else:
        print("\nAll recorded parameters varied across runs.")

    # 2) Top 3 ranked parameters (from overall ranking)
    top3 = []
    if "overall_rank" in results and isinstance(results["overall_rank"], pd.DataFrame) and len(results["overall_rank"]) > 0:
        top3 = [FEATURE_LABELS.get(i, i) for i in list(results["overall_rank"].index[:3])]
    if top3:
        print("Top 3 parameters with the strongest influence (based on current runs): " + ", ".join(top3) + ".")
