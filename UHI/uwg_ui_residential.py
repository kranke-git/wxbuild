...
def _find_rmy_epw_safe(wxbuild_root: Path, city: str) -> Path:
    """
    Pick the city RMY EPW from <wxbuild_root>/epwdata/<city>/rmy/.
    Prefer a filename containing the city key; otherwise fall back to the first EPW found.
    """
    rmy_dir = wxbuild_root / "epwdata" / city / "rmy"
    eps = sorted(rmy_dir.glob("*.epw"))
    if not eps:
    ...
    def on_run(_):
        with out:
            ...
            try:
                ...
                EPW_PATH = str(_find_rmy_epw_safe(wxbuild_root, city))
                ...
                ax.plot(base_t, label="Original RMY", color="0.25", linewidth=2)
                ax.plot(new_t,  label=f"UWG_Run{run_i}", color="#B11F3A", linewidth=2)

                ax.set_title("UWG vs RMY: Dry-Bulb Temperature")
