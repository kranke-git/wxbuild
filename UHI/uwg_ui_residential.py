...
def launch_ui(wxbuild_root: str = "/content/wxbuild", uwg_root: str = "/content/uwg"):
    ...
    # Low / Medium / High helper labels under selected sliders
    _lmh = "<div style=\"margin-left:140px; width:calc(100% - 140px); display:flex; justify-content:space-between; font-size:12px; color:#666;\"><span>Low</span><span>Medium</span><span>High</span></div>"
    blddensity_lbl = widgets.HTML(_lmh)
    grasscover_lbl = widgets.HTML(_lmh)
    treecover_lbl  = widgets.HTML(_lmh)

    urban_box = widgets.VBox([
        widgets.HTML("<h3>Urban Design</h3>"),

        landcov_title,
        widgets.VBox([treecover,  treecover_lbl], layout=widgets.Layout(margin="0 0 12px 0")),
        widgets.VBox([grasscover, grasscover_lbl], layout=widgets.Layout(margin="0 0 12px 0")),
        widgets.VBox([blddensity, blddensity_lbl], layout=widgets.Layout(margin="0 0 12px 0")),
        widgets.VBox([landcov_note], layout=widgets.Layout(margin="2px 0 14px 0")),

        widgets.VBox([bldheight], layout=widgets.Layout(margin="0 0 12px 0")),
        widgets.VBox([vertohor], layout=widgets.Layout(margin="0 0 12px 0")),
        widgets.VBox([albroad], layout=widgets.Layout(margin="0 0 12px 0")),
        widgets.VBox([sensanth], layout=widgets.Layout(margin="0 0 12px 0")),
    ], layout=col_layout)
