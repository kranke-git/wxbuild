        ...
        with out:
            ...
            try:
                ...
                print(f"Original EPW: {EPW_PATH}")
                             # Patch UWG template (no readDOE). We write a fresh patched .uwg file each run.
                detected_zone = detect_ashrae_zone_from_epw(epw_path) or "5A"
                
                # Ensure we have a baseline template .uwg to patch
                if not os.path.exists(PARAM_UWG):
                    build_initialize_residential(PARAM_UWG, default_zone=detected_zone)
                
                # Compute dtSim from timesteps/hour (UWG expects minutes)
                # If your UI already exposes dtSim directly, this will be overridden below.
                tsph = int(tsph) if "tsph" in locals() else 4
                dtSim_min = max(1, int(round(60 / tsph)))
                
                # Prepare updates (match keys used in initialize_residential.uwg)
                updates = {
                    "bldHeight": float(bldHeight),
                    "bldDensity": float(bldDensity),
                    "verToHor": float(verToHor),
                    "h_mix": float(h_mix),
                    "charLength": float(charLength),
                    "albRoad": float(albRoad),
                    "dRoad": float(dRoad),
                    "kRoad": float(kRoad),
                    "cRoad": float(cRoad),
                    "sensAnth": float(sensAnth),
                    "zone": str(detected_zone),
                    "grasscover": float(grasscover),
                    "treeCover": float(treeCover),
                    "vegStart": int(vegStart),
                    "vegEnd": int(vegEnd),
                    "albVeg": float(albVeg),
                    "latGrss": float(latGrss),
                    "latTree": float(latTree),
                    "rurVegCover": float(rurVegCover),
                    "dtSim": int(dtSim_min),
                    "month": int(month),
                    "day": int(day),
                    "nDay": int(nDay),
                ...
                patched_param_path = os.path.join(OUT_DIR, "initialize_residential_patched.uwg")
                patch_uwg_param_file(PARAM_UWG, patched_param_path, updates=updates, schtraffic=schtraffic)

                print(f"Param file: {patched_param_path}")

                # Run UWG
                m = UWG.from_param_file(patched_param_path, EPW_PATH, OUT_DIR, out_name)
