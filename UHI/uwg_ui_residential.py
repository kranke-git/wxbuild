...
# Local imports (works because you are in /content/wxbuild)
from UHI.uwg_setup_utils import (
    detect_cities,
    find_rmy_epw,
    ensure_city_uwg_dir,
...
def build_ui(wxbuild_root: str = "/content/wxbuild", uwg_root: str = "/content/uwg"):
    wxbuild_root = Path(wxbuild_root).resolve()
    uwg_root = Path(uwg_root).resolve()

    # --- Cities dropdown (auto-detect under epwdata) ---
    ...
    def on_run(_):
        with out:
            ...
            # keep these keys for UI completeness
...
# When run via %run, build UI immediately
build_ui()
