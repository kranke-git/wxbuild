"""wxbuild plotting helper: DBT + anomaly across SSPs.
...
def plot_dbt_plus_anomaly_all_ssps(
    ...
    *,
    ...
    """Plot annual dry-bulb temperature as (dbt + dbt_anomaly) for multiple SSPs.
    ...
    for ssp in ssps_list:
        ...
        y = df["dbt"].values + df["dbt_anomaly"].values
        ax.plot(df.index.values, y, label=ssp, linewidth=linewidth, color=colors.get(ssp))

    loc_txt = location or ""
    ax.set_title(f"{loc_txt} — dbt + anomaly {title_suffix}".strip())
    ax.set_xlabel("Year")
