...
def resolve_repo_location_folder(
    ...
    *,
    ...
    Supports repo subfolder name: "frmy" or "frmys".
    Strict: no cross-city fallback. Deterministic best match if multiple.

    Matching order:
      1) exact normalized match
      2) substring normalized match
      3) city-token prefix match (first token before underscore)
    If require_frmys=True, the candidate must contain frmy(s)/*.epw.
    """
