...
def show_frmy_picker(
    *,
    ...
    # If no base EPW exists yet, let the user pick a scenario and use "Select Scenario EPW"
    # to create one. Plotting will be enabled once a base EPW is available.

    base_dd = widgets.Dropdown(
        options=base_eps,
        value=(base_eps[0] if base_eps else None),
        description="Base EPW",
        layout=widgets.Layout(width="520px"),
        style={"description_width": "100px"},
    )

    base_note = widgets.HTML("")
    if not base_eps:
        base_dd.disabled = True
        base_dd.layout.display = None
        base_note.value = "<i>No base EPW found in Chosen_scenario. Select a scenario, then click <b>Select Scenario EPW</b> to set the base.</i>"
    elif len(base_eps) == 1:
        base_dd.layout.display = "none"

    def _base_path() -> str | None:
        if not base_dd.value:
            return None
    ...
    def _plot_current():
        ...
        with out:
            ...
            if base_path is None:
                print("⚠️ No base EPW selected yet. Click \"Select Scenario EPW\" to save one to Chosen_scenario.")
                display(select_btn)
                return
    ...
    display(widgets.VBox([header, base_dd, base_note, grid, out]))
