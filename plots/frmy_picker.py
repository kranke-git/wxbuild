...
def show_frmy_picker(
    *,
    ...
    # Widen buttons and prevent filename truncation
    display(HTML("""
    <style>
      .frmy-scenario-btn button{
    ...
    def _ssp_label(ssp_key: str) -> str:
        return {
            "ssp126": "SSP 1-26",
            "ssp245": "SSP 2-45",
            "ssp585": "SSP 5-85",
    ...
    def _ssp_key_from_name(name: str):
        n = name.lower()
        if "ssp126" in n: return "ssp126"
        if "ssp245" in n: return "ssp245"
        if "ssp585" in n: return "ssp585"
        return None

    # Group entries by SSP (and keep a fallback bucket)
    groups = {"ssp126": [], "ssp245": [], "ssp585": []}
    other = []
    for label, path in entries:
        key = _ssp_key_from_name(label)
        if key in groups:
            groups[key].append((label, path))
        else:
            other.append((label, path))

    # Build buttons + sections (SSP headings + grids)
    buttons = []
    def _make_grid(group_entries):
        grid_buttons = []
        for label, path in group_entries:
            b = widgets.Button(
                description=label,
    ...
    for key in ["ssp126", "ssp245", "ssp585"]:
        if groups[key]:
            sections.append(widgets.HTML(f"<b>{_ssp_label(key)}</b>"))
            sections.append(_make_grid(groups[key]))

    if other:
        sections.append(widgets.HTML("<b>Other</b>"))
        sections.append(_make_grid(other))
