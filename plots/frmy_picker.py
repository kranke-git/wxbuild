...
def resolve_repo_location_folder(
    ...
    *,
    ...
    """
    Resolve the *city folder name inside the repo* (repo_root/data/<CITY>/) that corresponds to the
    selected epwdata location (e.g., 'Phoenix_AZ_USA').

    Matching logic (strict, no cross-city fallback):
    1) Exact match on normalized strings (remove non-alphanumerics, lowercase)
    2) Substring match on normalized strings (either contains the other)
    If require_frmys=True, the resolved folder must contain frmys/*.epw.

    Raises a clear error if no match is found.
    """
    ...
    def has_frmys(folder: str) -> bool:
        # Support both folder names used in the repo: "frmy" and "frmys"
    ...
    # Filter by frmys presence if required
    if require_frmys:
    ...
    if len(exact) == 1:
        return exact[0]
    if len(exact) > 1:
        # deterministic: choose shortest name
    ...
    if len(sub) == 1:
        return sub[0]
    if len(sub) > 1:
        # deterministic: choose closest length to target then alphabetically
    ...
    raise RuntimeError(
        ...
        "Tip: ensure the repo has data/<CITY>/frmys/*.epw for this city, and that the folder name resembles the location key."
    )
...
def copy_frmys_to_epwdata_city(
    *,
    ...
    Sync FRMY EPWs from the *repo* city folder into the *epwdata* city folder:
        ...
        Source: <repo_root>/data/<CITY>/frmys/*.epw
        Dest:   <epwdata_root>/<location>/frmys/*.epw

    This ensures the FRMY picker always uses the FRMY set for the currently selected city
    from the GitHub repo, not whatever happens to already be in epwdata.

    If overwrite=True (default), destination EPWs are replaced each time.
    """
    if location is None:
    ...
    # Resolve repo city folder (strict match, no fallback)
    ...
    # Otherwise, copy from repo/data/<repo_loc>/frmys
    repo_loc = resolve_repo_location_folder(location, repo_root=repo_root, require_frmys=True)
    src_frmys = os.path.join(repo_root, "data", repo_loc, "frmys")
    if not os.path.isdir(src_frmys):
        src_frmys = os.path.join(repo_root, "data", repo_loc, "frmy")
...
def show_frmy_picker(
    *,
    ...
    """Display buttons for FRMY files and plot selected FRMY vs TMY. Adds a selector button to persist a chosen scenario EPW."""
    if location is None:
        location = globals().get("location")
    if not location:
    ...
    # 3) fallback: match by city token (first part before underscore), e.g., "Phoenix"
    city_token = location.split("_")[0].lower() if "_" in location else location.lower()
...
raise RuntimeError(f"No FRMY EPWs found in {frmy_dir}")
    ...
    def _set_active(active_btn):
        for bb in buttons:
            ...
            try:
                ...
                pass
        # Dark grey active + white text via CSS class
        active_btn.button_style = ""
        active_btn.style.button_color = "#B0B0B0"
        try:
    ...
    header = widgets.HTML(
        value=f"<b>FRMY files detected:</b> {len(entries)} &nbsp; | &nbsp; "
              f"<b>City:</b> {location} &nbsp; | &nbsp; "
              f"<b>RMY:</b> {os.path.basename(rmy_path)}"
    )
