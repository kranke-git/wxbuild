...
# (minimal change) use full filename so button text matches the EPW name
entries = []
for p in frmy_files:
    ...
    for label, path in entries:
        b = widgets.Button(description=label, layout=widgets.Layout(width="420px"))
...
# Group buttons by SSP (subheadings)
def _ssp_heading_from_path(p: str) -> str:
    scen, _, _ = _parse_frmy_filename(p)
    if scen == "ssp126":
        return "SSP 1-26"
    if scen == "ssp245":
        return "SSP 2-45"
...
grouped = {"SSP 1-26": [], "SSP 2-45": [], "SSP 5-85": [], "Other": []}
for b, (lab, pth) in zip(buttons, entries):
    grouped[_ssp_heading_from_path(pth)].append(b)

sections = []
for heading in ["SSP 1-26", "SSP 2-45", "SSP 5-85", "Other"]:
    btns = grouped.get(heading, [])
    if not btns:
        continue
    sections.append(widgets.HTML(value=f"<b>{heading}</b>"))
    sections.append(
        widgets.GridBox(
            btns,
            layout=widgets.Layout(
                grid_template_columns="repeat(2, 420px)",
                grid_gap="8px 10px"
            )
        )
    )
