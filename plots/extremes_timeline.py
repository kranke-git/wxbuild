...
Repo expectations
-----------------
Repo root (default): /content/wxbuild
City extremes folder: <repo_root>/data/<LOCATION>/extremes/
"""
...
def _auto_detect_location_from_repo(repo_root: str = "/content/wxbuild") -> str:
    ...
    # prefer those with extremes files
...
def copy_extremes_to_colab(
    *,
    ...
    src_dir = os.path.join(repo_root, "data", location, "extremes")
    if not os.path.isdir(src_dir):
        raise RuntimeError(f"Extremes folder not found for location '{location}': {src_dir}")
